...
import { attack, heal, magic, playerStats, wait } from '../shared';

export const useBattleSequence = (sequence, foe, pClass) => {
  const [turn, setTurn] = useState(0);
  const [inSequence, setInSequence] = useState(false);
  const [playerHealth, setPlayerHealth] = useState(pClass.maxHealth);
  const [opponentHealth, setOpponentHealth] = useState(foe.maxHealth);
  ...
  useEffect(() => {
    ...
    if (mode) {
      ...
      const attacker = turn === 0 ? pClass : foe;
      const receiver = turn === 0 ? foe : pClass;
